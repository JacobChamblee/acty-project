...
# Keywords found in OBD adapter device names (case-insensitive)
_OBD_KEYWORDS = {"obd", "veepeak", "obdii", "elm327", "obdcheck", "vlink", "kiwi", "carista"}

def bt_find_obd(scan_timeout: int = 10) -> str | None:
    """Auto-detect an OBD Bluetooth adapter MAC address.

    1. Checks already-paired / cached devices immediately (no scan needed).
    2. If nothing found, does a brief bluetoothctl scan for new devices.

    Returns the MAC string (e.g. '8C:DE:52:D9:7E:D1') or None.
    """
    import re as _re
    _mac_re = _re.compile(r"([0-9A-F]{2}(?::[0-9A-F]{2}){5})", _re.IGNORECASE)

    def _check_line(line: str) -> str | None:
        """Return MAC if the line describes a known OBD device, else None."""
        m = _mac_re.search(line)
        if not m:
            return None
        mac = m.group(1).upper()
        name_part = line[m.end():].lower().strip()
        if any(k in name_part for k in _OBD_KEYWORDS):
            return mac
        return None

    # ── Step 1: cached / paired devices (instant) ────────────────────────────
    try:
        ...
        for line in result.stdout.splitlines():
            mac = _check_line(line)
            if mac:
                print(f"[BT] Found OBD device (cached): {mac}  ({line.strip()})")
                return mac
    except Exception as e:
        print(f"[BT] Warning: bluetoothctl devices failed: {e}")

    # ── Step 2: active scan ───────────────────────────────────────────────────
    print(f"[BT] No cached OBD device found — scanning {scan_timeout}s for new devices...")
    try:
        ...
        while time.monotonic() < deadline:
            ...
            proc.stdin.flush()
            time.sleep(1.0)
            # Read whatever output is buffered
            import select as _select
            ready, _, _ = _select.select([proc.stdout], [], [], 0.1)
            if ready:
                lines = []
                while True:
                    r2, _, _ = _select.select([proc.stdout], [], [], 0.05)
                    if not r2:
                        break
                ...
                for line in lines:
                    ...
                    if mac:
                        found_mac = mac
                        print(f"[BT] Found OBD device (scan): {mac}  ({line.strip()})")
                        break
            if found_mac:
                break
        ...
        print(f"[BT] Warning: scan failed: {e}")
        return None
...
class ELM327:
    ...
    def get_vin(self) -> str | None:
        """Query Mode 09 PID 02 for the 17-character VIN.
        Returns the VIN string, or None if the vehicle doesn't support it."""
        resp = self._send("0902", timeout=5.0)
        if not resp or any(x in resp.upper() for x in ["NO DATA", "ERROR", "UNABLE", "?"]):
            return None
        # Strip all whitespace and find the 4902 response header
        hex_str = resp.upper().replace(" ", "").replace("\n", "")
        idx = hex_str.find("4902")
        if idx >= 0:
            hex_str = hex_str[idx + 4:]
        # Skip the first byte (number of data items) if present
        # VIN is 17 ASCII chars = 34 hex chars
        # Find the first printable ASCII run of 17 chars
        try:
            even_len = len(hex_str) - (len(hex_str) % 2)  # trim to even length for fromhex
            raw_bytes = bytes.fromhex(hex_str[:even_len])
            chars = "".join(chr(b) for b in raw_bytes if 0x20 <= b <= 0x7E)
            # VIN is always 17 alphanumeric characters
            import re
            match = re.search(r'[A-HJ-NPR-Z0-9]{17}', chars.upper())
            if match:
...
class DataLogger:
    def __init__(self, fmt, log_file, pids, vin=None):
        self.fmt = fmt
        ...
        if log_file and fmt == "csv":
            ...
            fields = ["timestamp", "elapsed_s", "VIN"] + pids
...
def main():
    ...
    parser.add_argument("--address", "-a", default=None,
        help="Bluetooth MAC address (auto-detected if omitted)")
    ...
    if address is None:
        print("[BT] No --address given; attempting auto-detection...")
        address = bt_find_obd(scan_timeout=12)
        if address is None:
            print("[ERROR] Could not auto-detect an OBD Bluetooth adapter.")
            print("  • Is the dongle plugged in with ignition ON?")
            print("  • Try pairing first: bluetoothctl → scan on → pair <MAC>")
            print("  • Or pass --address XX:XX:XX:XX:XX:XX explicitly")
            sys.exit(1)
    else:
        print(f"[BT] Using specified address: {address}")

    # ── Connect ───────────────────────────────────────────────────────────────
    print(f"[BT] Connecting to {address}...")
    try:
        sock = bt_connect(address)
    except OSError as e:
        print(f"\n[ERROR] {e}")
    ...
    # ── VIN query ─────────────────────────────────────────────────────────────
    print("[VIN] Querying vehicle VIN (Mode 09 PID 02)...")
    vin = elm.get_vin()
    if vin:
        print(f"[VIN] {vin} ✓")
    else:
        print("[VIN] Not available — vehicle may not support Mode 09")
    print()
    if args.dtc:
    ...
    if not args.no_log:
        ...
        data_dir = Path.home() / "acty-project" / "data_capture"
        data_dir.mkdir(parents=True, exist_ok=True)
        log_file = data_dir / f"acty_obd_{ts}.{ext}"

    logger = DataLogger(fmt=args.format, log_file=log_file, pids=pids, vin=vin)
